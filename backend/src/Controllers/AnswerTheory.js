...
const replicate = new Replicate({
  auth: process.env.AI_KEY || "r8_7b0ee1Z9sPwR3j2yGfHhZCCExjWrNqv2HtnjQ"
});
...
export const submitTheoryAnswer = async (req, res) => {
    ...
    try {
        ...
        console.log("the marking guide=> "+ JSON.stringify(marking_guide)+ "my answer =>" + answerText)
        ...
        console.log("the data sent => "+question+ " ------- " + score + "------- "+ theoryPercentage+ "------ "+ feedback)
        res.status(201).json({ message: 'Answer submitted successfully', question, score,theoryPercentage, feedback });
    } catch (error) {
